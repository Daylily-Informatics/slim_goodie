...
if [[ "$mode" == "dev" ]]; then
  ...
  uvicorn main:app \
    ...
    --host $host  \
    --ssl-certfile /etc/letsencrypt/live/bangateapot.wtf/fullchain.pem \
    --ssl-keyfile  /etc/letsencrypt/live/bangateapot.wtf/privkey.pem
  ...
  gunicorn main:app \
    ...
    --timeout 15 \
    --bind $host:$port \
    --certfile /etc/letsencrypt/live/bangateapot.wtf/fullchain.pem \
    --keyfile  /etc/letsencrypt/live/bangateapot.wtf/privkey.pem
else
