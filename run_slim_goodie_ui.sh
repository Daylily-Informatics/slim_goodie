...
if [ -z "$2" ]; then
  ...
  sleep 2
  uvicorn main:app --reload --log-level trace --port 8918 --timeout-keep-alive 303 --host $host
else
  echo "RUNNING IN PRODUCTION MODE"
  echo "Running with $num_workers workers on $host"
  sleep 4
  gunicorn main:app -w $num_workers -k uvicorn.workers.UvicornWorker --log-level trace --timeout 303 --bind $host:8918
fi
